function [centers] = beam2sphere(beam, size, col, T)
%function [s_array, centers, rs_list] = beam2sphere(beam, size, col, T)
% This function transform the coordinates of a beam into spheres
% Input:    
%           beam : e1 e2 e3 e4 e5 e6 e7 e8, coordinates of the edges. 
%           size : diameter of the spheres, mm
%           col  : FaceColor, e.g. 'r', [1,0,0],'#FF0000'
%           T    : Transformation matrix
% Output: 
%           s : array of surfaces from the spheres of the beam
...
% Radius is equal to the specified size of the sphere. 
rs = size; 
% Side of the cube
r = 2*rs/sqrt(3); 
